...
export async function POST(request: NextRequest) {
  try {
    const { customerId, storePin, itemId } = await request.json()

    if (!customerId || !storePin || !itemId) {
      return NextResponse.json({ error: "Customer ID, store PIN, and item ID are required" }, { status: 400 })
    }
    ...
    if (storePin !== settings.store_pin) {
      return NextResponse.json({ error: "Invalid store PIN" }, { status: 400 })
    }
